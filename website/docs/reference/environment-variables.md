...
| `HERMES_GIT_BASH_PATH` | **Windows only.** Override `bash.exe` discovery for the terminal tool. Point it at a native Windows bash such as Git-for-Windows, PortableGit, MSYS2, or Cygwin. Do not point it at WSL's `bash.exe`; Hermes rejects the WSL launcher on native Windows because cwd, process, and path semantics are different. The installer sets this automatically to the PortableGit it provisioned. See the [Windows (Native) Guide](../user-guide/windows-native.md#how-hermes-runs-shell-commands-on-windows) |
